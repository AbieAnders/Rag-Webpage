...
let embeddingsList: Float32Array[] = []; // To store embeddings
...
async function storeInSupabase(url: string, text: string, embedding: number[]) {
    try {
...
export async function POST(req: NextRequest) {
    try {
        ...
        await storeInSupabase(url, scrapedText, paragraphEmbeddingsAsArray);
